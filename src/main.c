...
showStockList () {
  ...
  stockData *item_t = (struct stockStruct *) malloc(BUFFER * sizeof(struct stockStruct));
...
searchItemId (stockData *item_t, int itemId, int infLim, int supLim) {
  ...
  while (init <= end) {
    ...
    if (item_t[middle].itemId == itemId) {
      return middle;
    } else if (item_t[middle].itemId > itemId) {
      end = middle - 1;
    } else {
      init = middle + 1;
    }
...
checkItemId (FILE *savedStockFile, stockData *item_t, int lines, int flag) {
  ...
  qsort(item_t, lines, sizeof(struct stockStruct), comparator);
